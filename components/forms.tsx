...
export function NewsletterForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (client) {
        await client.mutation(api.storefront.subscribeToNewsletter, {
  ...
  return (
    <form className="newsletter-form" onSubmit={submit}>
      ...
      <button type="submit" disabled={status === "submitting"} aria-label="Join the newsletter">
        <ArrowRight aria-hidden="true" />
      </button>
      <p className="form-message" aria-live="polite">
        {status === "success" && "Welcome to VELONA. A note is on its way."}
    ...
    try {
      ...
      if (client) {
        await client.mutation(api.storefront.submitWholesaleInquiry, {
          ...
          storeType: String(form.get("storeType")) || undefined,
  ...
  return (
    <form className="wholesale-form" onSubmit={submit}>
      ...
      <div className="form-row">
        ...
        <label><span>Business name</span><input name="businessName" required autoComplete="organization" /></label>
      </div>
      <div className="form-row">
      ...
      <label>
        ...
        <textarea name="message" rows={3} required placeholder="Location, aesthetic, and the pieces you are interested in…" />
      </label>
      <button className="button button-ivory" type="submit" disabled={status === "submitting"}>
        {status === "submitting" ? "Sending…" : "Send inquiry"}<ArrowRight aria-hidden="true" />
      </button>
      <p className="form-message" aria-live="polite">
        {status === "success" && "Thank you. We’ll be in touch within 2–3 business days."}
